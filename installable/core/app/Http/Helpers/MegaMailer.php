...
class MegaMailer {
    ...
    public function mailFromAdmin($data) {
        ...
        if (array_key_exists('requested_domain', $data)) {
            $body = preg_replace("/{requested_domain}/", $data['requested_domain'], $body);
        }
        if (array_key_exists('previous_domain', $data)) {
            $body = preg_replace("/{previous_domain}/", $data['previous_domain'], $body);
        }
        if (array_key_exists('current_domain', $data)) {
            $body = preg_replace("/{current_domain}/", $data['current_domain'], $body);
        }
        if (array_key_exists('subdomain', $data)) {
            $body = preg_replace("/{subdomain}/", $data['subdomain'], $body);
        }
